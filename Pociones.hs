aplicar3 f (a, b, c) = (f a, f b, f c)
invertir3 (a, b ,c) = (c, b, a)

suerte (s, _, _) = s
convencimiento (_, c,_) = c
fuerzaFisica (_,_,ff) = ff

sinRepetidos [] = []
sinRepetidos (x:xs)
    |elem x xs = sinRepetidos xs
    |otherwise = x : sinRepetidos xs

maximoF _ [ x ] = x
maximoF f (x: y : xs)
    |f x > f y = maximoF f (x:xs)
    |otherwise = maximoF f (y:xs)

--suerte, convenciomiento, fuerzaFisica
type Persona = (String, (Int,Int,Int))

--podria ser tupla
data Pocion = Pocion {
    nombre :: String,
    ingredientes :: [Ingrediente]
}  deriving 

data Ingrediente = Ingrediente {
    nombreIngrediente :: String,
    gramos :: Int,
    efectos:: [Efecto]
} 

type Efecto = (Int,Int,Int) ->  (Int,Int,Int)

-- Ejemplos
harry::Persona
harry = ( "Harry", (11,5,4))
ron::Persona
ron = ("Ron", (6,4,6) )
hermione::Persona
hermione = ("Hermione", (8,12,2))
draco::Persona
draco = ("Draco",(7,9,6))

felixFelices= Pocion "Felix Felices" [escarabajosMachacados, ojoDeTigreSucio]
multiJugos = Pocion "Multijugos" [cuernoDeBicornioEnPolvo, sanguijuelaHormonal]
floresDeBach = Pocion "Flores De Bach" [orquideaSalvaje, rosita]

escarabajosMachacados = Ingrediente "Escarabajos Machacados" 52 [f1, f2]
ojoDeTigreSucio = Ingrediente "Ojo de Tigre Sucio" 2 [f3]
cuernoDeBicornioEnPolvo = Ingrediente "Cuerno de Bicornio en Polvo" 10
   [invertir3, (\(a, b, c) -> (a, a, c))]
sanguijuelaHormonal = Ingrediente "Sanguijuela Hormonal" 54 [aplicar3 (*2),
   (\(a, b, c) -> (a, a, c))]
orquideaSalvaje = Ingrediente "Orquídea Salvaje" 8 [f3]
rosita = Ingrediente "Rosita" 1 [f1]



f1 (ns, nc, nf) = (ns+1, nc+2, nf+3)
f2 = aplicar3 (max 7)
f3 (ns, nc, nf)
  | ns >= 8 = (ns, nc, nf+5)
  | otherwise = (ns, nc, nf-3)


  --Punto 2
--Mejor haber usado las funciones que te prestaron arriba y tambien derivar
sumaNiveles :: (Int,Int,Int) -> Int
sumaNiveles (s,c,ff) = s + c + ff

diferenciaNiveles :: (Int, Int, Int) -> Int
diferenciaNiveles (s,c,ff) = (max (max s c)ff) - (min (min s c)ff) --Derivarlo

--Punto 3
sumaNivelesPersona :: Persona -> Int
sumaNivelesPersona (nombre,niveles) = sumaNiveles niveles

diferenciaNivelesPersona :: Persona -> Int
diferenciaNivelesPersona (nombre , niveles) = diferenciaNiveles niveles

--punto 4

efectosDePocion :: Pocion -> [Efecto]
efectosDePocion pocion = concat.map (efectos). ingredientes $ pocion

--Punto 5
pocionesHeavies :: [Pocion] -> [String] 
pocionesHeavies [] = []
pocionesHeavies (x:xs) 
    | (length (efectosDePocion x) )> 3 = nombre x : pocionesHeavies xs
    | otherwise = pocionesHeavies xs
 
--Punto 6 -- no se me ocurrio el a
incluyeA:: Eq a => [a] ->[a]-> Bool
incluyeA lista lista2 = all (flip elem lista2) lista

esPocionMagica :: Pocion -> Bool
esPocionMagica pocion = (incluyeA "aeiuo" (nombre pocion)) && (all (even.gramos) (ingredientes pocion))

--Punto 7
tomarPocion :: Persona -> Pocion -> Persona
tomarPocion (nombre,niveles) pocion = (nombre ,(foldl (flip ($)) (niveles) (efectosDePocion pocion)))